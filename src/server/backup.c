...
// execute commands for a backup server
...
int main(int argc, char *argv[]) {
  ...
  send(sockfd, "backup", 6, 0);

  // start a recv thread for backup
  pthread_t tid = 0;
...
void *handleBackupRecv() {
  ...
  while ((len = recv(leaderInfo.sockfd, buf, 1024, 0)) > 0) {
    ...
    int res =
        parseAndExecute(buf, len); // Parse and execute command on backup server
    printf("%d:%s\n", len, buf);
